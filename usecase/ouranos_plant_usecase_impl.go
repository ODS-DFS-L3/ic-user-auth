...
func (u *plantUsecase) PutPlant(plantModel traceability.PlantModel) (traceability.PlantModel, error) {
	if plantModel.Attribute.PlantID == uuid.Nil {
		// If pm.PlantID does not exist, create a new one.
		e := traceability.NewPlantEntityModel(
			plantModel.Attribute.OperatorID,
			plantModel.Attribute.PlantName,
			plantModel.Attribute.PlantAddress,
			plantModel.Attribute.OpenPlantID,
			plantModel.Attribute.PlantAttribute.GlobalPlantID,
		)
		...
		if err != nil {
			...
			if errors.As(err, &pgError) {
				if pgError.Code == common.PgErrorUniqueViolation {
					...
					msg := common.DuplicatePlantError(pgError, plantModel.Attribute.OpenPlantID, plantModel.Attribute.PlantAttribute.GlobalPlantID)
		...
		e, err := u.r.GetPlant(plantModel.Attribute.OperatorID.String(), plantModel.Attribute.PlantID.String())
		if err != nil {
		...
		if err != nil {
			...
			if errors.As(err, &pgError) {
				if pgError.Code == common.PgErrorUniqueViolation {
					// Unique constraint error
					msg := common.DuplicatePlantError(pgError, e.OpenPlantID, plantModel.Attribute.PlantAttribute.GlobalPlantID)
