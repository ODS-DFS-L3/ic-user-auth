	...
	if getOperatorInput.OpenOperatorID != nil {
		...
		if err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				// If the operatorID corresponding to the DB does not exist, return a normal response.
				return traceability.OperatorModel{}, nil
			}
...
func (u *operatorUsecase) PutOperator(operatorModel traceability.OperatorModel) (traceability.OperatorModel, error) {
	e, err := u.r.GetOperator(operatorModel.Attribute.OperatorID.String())
	if err != nil {
		logger.Set(nil).Errorf(err.Error())
	...
	if err != nil {
		...
		if errors.As(err, &pgError) {
			if pgError.Code == common.PgErrorUniqueViolation {
				...
				msg := common.DuplicateOperatorError(pgError, operatorModel.Attribute.OperatorAttribute.GlobalOperatorID)
