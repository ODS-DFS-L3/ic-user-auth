...
// StringEmptyOrUUIDValid
// Summary: This is function which checks whether the string is a valid UUID. Empty characters are allowed.
// input: value (interface{})
// output: (error) error object
func StringEmptyOrUUIDValid(value interface{}) error {
	s, _ := value.(string)
	if s == "" {
		return nil
	} else {
