...
func SetRouter(e *echo.Echo, h handler.AppHandler, config *config.Config, conn *gorm.DB, authMiddleware custom_middleware.AuthMiddleware) {
	...
	e.GET("/api/v2/authInfo/health", func(c echo.Context) error { return h.HealthCheck(c) })
	...
	auth.POST("/client", func(c echo.Context) error { return h.Client(c) })
	...
	systemAuth := authGroup.Group("/api/v2/systemAuth")
	...
	authInfo := authGroup.Group("/api/v2/authInfo")
	authInfo.Use(authMiddleware.AuthJWT())
	authInfo.GET("/operator", func(c echo.Context) error { return h.GetOperator(c) })
	authInfo.PUT("/operator", func(c echo.Context) error { return h.PutOperator(c) })
	authInfo.GET("/plant", func(c echo.Context) error { return h.GetPlant(c) })
	authInfo.PUT("/plant", func(c echo.Context) error { return h.PutPlant(c) })
}
