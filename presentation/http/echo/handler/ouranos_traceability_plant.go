...
func (h *plantHandler) GetPlant(c echo.Context) error {
	...
	if err != nil {
		...
		return echo.NewHTTPError(common.HTTPErrorGenerate(http.StatusUnauthorized, common.HTTPErrorSourceAuth, common.Err401InvalidToken, operatorID, dataTarget, method))
	}
...
func (h *plantHandler) PutPlant(c echo.Context) error {
	var putPlantInput traceability.PlantModelInput
	dataTarget := "plant"
	method := c.Request().Method
	operatorID := c.Get("operatorID").(string)
	...
	if OperatorUUID.String() != putPlantInput.Attribute.OperatorID {
