...
func (h *operatorHandler) GetOperator(c echo.Context) error {
	...
	var r []traceability.OperatorModel
	if reflect.DeepEqual(m, traceability.OperatorModel{}) {
		r = []traceability.OperatorModel{}
	} else {
		r = []traceability.OperatorModel{m}
	}
...
func (h *operatorHandler) PutOperator(c echo.Context) error {
	var i traceability.OperatorModelInput
	dataTarget := "operator"
	method := c.Request().Method
	...
	if operatorUUID != om.Attribute.OperatorID {
