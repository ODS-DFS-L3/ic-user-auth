...
// Client
// Summary: This is function which is used to client login
// input: c(echo.Context): context
// output: error: error object
func (h *authHandler) Client(c echo.Context) error {
	method := c.Request().Method
	param := input.ClientParam{}
	...
	output, err := h.AuthUsecase.Client(param)
	if err != nil {
		var customErr *common.CustomError
		if errors.As(err, &customErr) {
			if customErr.IsWarn() {
